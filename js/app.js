...
const notify = (title, body) => {
  ...
  if (Notification.permission === 'granted') {
    ...
    Notification.requestPermission().then(p => {
      if (p === 'granted') new Notification(title, { body, icon: 'assets/icon-192.png' });
    });
  }
};
...
const DataStore = {
  ...
  updatePayment(id, updates) {
    const payments = this.getPayments().map(p => p.id === id ? { ...p, ...updates } : p);
    this.savePayments(payments);
    return payments;
  },
    ...
    <div className="sidebar">
      <div className="sidebar-brand">
        ...
        <div className="brand-text">
          <h3>VoteTrack</h3>
          <span>TAC Budget Monitoring</span>
        </div>
      </div>
      ...
      <div className="sidebar-footer">
        <div>TAC VoteTrack v1.1</div>
            <div className="version">Financial Year {APP_CONFIG.fiscalYear}</div>
      </div>
    </div>
  );
}
    ...
    <div className="topbar">
      ...
      <div className="topbar-actions">
        <div className="user-chip">
          ...
          <div>
            <div className="user-name">TAC Planning Team</div>
    ...
    <div>
      <div className="stats-grid">
        ...
        <div className="stat-card">
          ...
          <div className="stat-sub">Financial Year {APP_CONFIG.fiscalYear} allocation</div>
        </div>
        <div className="stat-card">
...
function PaymentsPage() {
  ...
  const [form, setForm] = useState({ vote_code: '', payment_date: '', payment_reference: '', payee: '', description: '', amount: '', entered_by: 'TAC Planning Team', currency: 'UGX', exchange_rate: '1' });
  const [search, setSearch] = useState('');
  ...
  const handleSubmit = (e) => {
    ...
    const cur = form.currency || 'UGX';
    const rate = cur !== 'UGX' ? (parseFloat(form.exchange_rate) || 1) : 1;
    const ugxAmount = cur !== 'UGX' ? Math.round(amount * rate) : amount;
    const currentSpent = spentMap[form.vote_code] || 0;
    const newBalance = vote.initial_budget - currentSpent - ugxAmount;

    if (newBalance < 0) {
      setWarnOverspend({ ...form, amount: ugxAmount, overspendAmount: Math.abs(newBalance) });
      return;
    }
  ...
  const savePayment = (data) => {
    ...
    const rate = cur !== 'UGX' ? (parseFloat(data.exchange_rate) || 1) : 1;
    const origAmt = parseFloat(data.amount);
    const ugxAmount = cur !== 'UGX' ? Math.round(origAmt * rate) : origAmt;
    ...
    if (editingPayment) {
      const updates = {
        ...
        amount: ugxAmount,
        currency: cur,
        original_amount: cur !== 'UGX' ? origAmt : null,
        exchange_rate: cur !== 'UGX' ? rate : null,
        entered_by: data.entered_by || 'TAC Planning Team'
      };
      const updated = DataStore.updatePayment(editingPayment.id, updates);
      setPayments(updated);
      if (FirebaseAPI.isConfigured()) {
        FirebaseAPI.deletePayment(editingPayment.id);
        FirebaseAPI.addPayment({ ...updates, id: editingPayment.id, created_at: editingPayment.created_at });
      }
      setShowForm(false);
      setEditingPayment(null);
      setWarnOverspend(null);
      setForm({ vote_code: '', payment_date: '', payment_reference: '', payee: '', description: '', amount: '', entered_by: 'TAC Planning Team', currency: 'UGX', exchange_rate: '1' });
      setToast({ message: `Payment updated for ${updates.vote_code}`, type: 'success' });
      notify('Payment Updated', `Payment to "${data.payee}" on vote ${data.vote_code} was edited.`);
      return;
    }
    ...
    setWarnOverspend(null);
    setForm({ vote_code: '', payment_date: '', payment_reference: '', payee: '', description: '', amount: '', entered_by: 'TAC Planning Team', currency: 'UGX', exchange_rate: '1' });
    const amtLabel = cur !== 'UGX' ? `${cur} ${fmt(origAmt)} (UGX ${fmt(ugxAmount)})` : `UGX ${fmt(ugxAmount)}`;
    setToast({ message: `Payment of ${amtLabel} recorded for ${payment.vote_code}`, type: 'success' });
    notify('Payment Recorded', `${amtLabel} to "${data.payee}" on vote ${payment.vote_code}`);
  };
  ...
  const startEdit = (p) => {
    setEditingPayment(p);
    setForm({
      vote_code: String(p.vote_code),
      payment_date: p.payment_date,
      payment_reference: p.payment_reference,
      payee: p.payee,
      description: p.description || '',
      amount: p.original_amount != null ? String(p.original_amount) : String(p.amount),
      entered_by: p.entered_by,
      currency: p.currency || 'UGX',
      exchange_rate: p.exchange_rate != null ? String(p.exchange_rate) : '1'
    });
    setShowForm(true);
  };
    ...
    <div>
      ...
      <div className="filters-bar">
        ...
        <div style={{ marginLeft: 'auto', paddingBottom: 4 }}>
          <button className="btn btn-primary" onClick={() => { setShowForm(true); setEditingPayment(null); setWarnOverspend(null); }}>
              ...
              {filtered.map(p => (
                <tr key={p.id}>
                  ...
                  <td className="num" style={{ fontWeight: 700, color: 'var(--danger)' }}>
                    {p.currency && p.currency !== 'UGX'
                      ? <span title={`${p.currency} ${fmt(p.original_amount || p.amount)} @ rate ${p.exchange_rate}`}>{fmt(p.amount)} <span style={{ fontSize: 10, color: 'var(--text-muted)' }}>({p.currency})</span></span>
                      : fmt(p.amount)}
                  </td>
                  <td style={{ fontSize: 12, color: 'var(--body)' }}>{p.entered_by}</td>
                  <td>
                    <button className="btn btn-secondary btn-sm" onClick={() => startEdit(p)} title="Edit payment" style={{ marginRight: 4 }}><i className="fas fa-pen" /></button>
                    <button className="btn btn-danger btn-sm" onClick={() => setConfirmDel(p.id)} title="Delete payment"><i className="fas fa-trash" /></button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {showForm && (
        <Modal title={editingPayment ? 'Edit Payment' : 'Record Payment'} onClose={() => { setShowForm(false); setEditingPayment(null); setWarnOverspend(null); }}>
          <form onSubmit={handleSubmit}>
            ...
            <div className="form-row">
              ...
              <div className="form-group">
                ...
                <select value={form.currency} onChange={e => setForm({ ...form, currency: e.target.value, exchange_rate: e.target.value === 'UGX' ? '1' : form.exchange_rate })}>
            ...
            {form.currency !== 'UGX' && (
              <div className="form-row">
                <div className="form-group">
                  ...
                  <input type="number" min="1" step="any" value={form.amount} onChange={e => setForm({ ...form, amount: e.target.value })} required placeholder="Enter foreign amount" />
                </div>
                <div className="form-group">
                  <label>Exchange Rate (1 {form.currency} = ? UGX) *</label>
                  <input type="number" min="0.01" step="any" value={form.exchange_rate} onChange={e => setForm({ ...form, exchange_rate: e.target.value })} required placeholder="e.g. 3750" />
                </div>
              </div>
            )}
            {form.currency === 'UGX' && (
            ...
            {form.currency !== 'UGX' && form.amount && form.exchange_rate && (
              <div style={{ background: '#f0f9ff', padding: 8, borderRadius: 6, marginBottom: 12, fontSize: 12, color: '#1d4ed8', fontWeight: 600 }}>
                <i className="fas fa-calculator" style={{ marginRight: 6 }} />
                Equivalent: UGX {fmt(Math.round(parseFloat(form.amount) * (parseFloat(form.exchange_rate) || 1)))}
              </div>
            )}
            <div className="form-row">
              <div className="form-group">
            ...
            <div className="form-group">
              ...
              <input type="text" value={form.entered_by} onChange={e => setForm({ ...form, entered_by: e.target.value })} placeholder="Your name" />
            </div>
            <div className="modal-footer">
              <button type="button" className="btn btn-secondary" onClick={() => { setShowForm(false); setEditingPayment(null); setWarnOverspend(null); }}>Cancel</button>
              <button type="submit" className="btn btn-primary"><i className="fas fa-check" /> {editingPayment ? 'Update Payment' : 'Record Payment'}</button>
            </div>
          </form>
        </Modal>
      )}
...
function ReportsPage() {
  ...
  const getEffectiveDates = () => {
    ...
    if (dateRange === 'month') return { from: now.getFullYear() + '-' + String(now.getMonth() + 1).padStart(2, '0') + '-01', to: today };
  ...
  const reportPayments = useMemo(() => {
    ...
    let list = payments;
    if (from) list = list.filter(p => p.payment_date >= from);
    if (to) list = list.filter(p => p.payment_date <= to);
  ...
  const exportPDF = () => {
    ...
    const doc = new jsPDF();
    doc.setFontSize(16);
    doc.text('TAC Budget Monitoring', 14, 18);
    doc.setFontSize(10);
    doc.text(`Report: ${reportType === 'summary' ? 'Vote Summary' : 'Payment Transaction'}`, 14, 26);
    doc.text(`Generated: ${new Date().toLocaleString()} | Financial Year ${APP_CONFIG.fiscalYear}`, 14, 32);
    ...
    if (reportType === 'summary') {
      ...
      const rows = reportPayments.map(p => {
        ...
        const amt = cur !== 'UGX' ? `${cur} ${fmt(p.original_amount || p.amount)} (UGX ${fmt(p.amount)})` : fmt(p.amount);
        return [fmtDate(p.payment_date), p.payment_reference, p.vote_code, p.payee, amt];
      });
        doc.autoTable({
          startY: 38, head: [['Date', 'Ref', 'Vote', 'Payee', 'Amount']], body: rows,
          theme: 'grid', headStyles: { fillColor: [0, 102, 51] }, fontSize: 8,
          columnStyles: { 4: { halign: 'right', cellWidth: 45 } }
        });
    }
    doc.text(`Prepared by: TAC VoteTrack`, 14, doc.internal.pageSize.height - 10);
    doc.save(`TAC-Report-${reportType}-${new Date().toISOString().slice(0, 10)}.pdf`);
    notify('Report Generated', `${reportType === 'summary' ? 'Vote Summary' : 'Payment Transactions'} report exported as PDF.`);
  };
  ...
  const exportCSV = () => {
    ...
    if (reportType === 'summary') {
      ...
      csv = 'Date,Reference,Vote Code,Payee,Description,Currency,Original Amount,Rate,Amount (UGX),Entered By\n';
      reportPayments.forEach(p => {
        const cur = p.currency || 'UGX';
        const orig = p.original_amount != null ? p.original_amount : p.amount;
        const rate = p.exchange_rate != null ? p.exchange_rate : 1;
        csv += `"${p.payment_date}","${p.payment_reference}","${p.vote_code}","${p.payee}","${p.description}","${cur}",${orig},${rate},${p.amount},"${p.entered_by}"\n`;
      });
    }
    const blob = new Blob([csv], { type: 'text/csv' });
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = `TAC-${reportType}-${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
    notify('Report Generated', `${reportType === 'summary' ? 'Vote Summary' : 'Payment Transactions'} report exported as CSV.`);
  };
      ...
      <div className="card no-print">
        <div style={{ textAlign: 'center', marginBottom: 16, paddingBottom: 12, borderBottom: '2px solid var(--stroke)' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, marginBottom: 6 }}>
            <img src="assets/ura-logo.png" alt="TAC" style={{ width: 32, height: 32 }} onError={e => e.target.style.display = 'none'} />
            <h3 style={{ fontSize: 16, fontWeight: 700 }}>Tax and Accounting Centre</h3>
          </div>
          <p style={{ fontSize: 13, color: 'var(--text-muted)' }}>{reportType === 'summary' ? 'Vote Summary Report' : 'Payment Transaction Report'} — Financial Year {APP_CONFIG.fiscalYear}</p>
